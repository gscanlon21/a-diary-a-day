...
public class UserService
{
    ...
    public async Task<IList<UserMoodDto>?> GetWorkouts(string email, string token)
    {
        return await _httpClient.GetFromJsonAsync<List<UserMoodDto>>($"{_siteSettings.Value.ApiUri.AbsolutePath}/User/Workouts?email={Uri.EscapeDataString(email)}&token={Uri.EscapeDataString(token)}");
    }
}
