...
public class UserRepo(CoreContext context)
{
    ...
    {
        var lastDeload = await _context.UserMoods.AsNoTracking().TagWithCallSite()
    ...
    /// </summary>
    public async Task<UserMood?> GetCurrentWorkout(User user)
    {
        return await _context.UserMoods.AsNoTracking().TagWithCallSite()
            .Where(n => n.UserId == user.Id)
