...
namespace Web.Views.Shared.Components.SerumNutrients;

public class SerumNutrientsViewModel
{
    public SerumNutrientsViewModel(IList<UserSerumNutrients>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumNutrients).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumNutrients UserMood { get; init; } = null!;
    public UserSerumNutrients? PreviousMood { get; init; }
