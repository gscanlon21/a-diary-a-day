...
namespace Web.Views.Shared.Components.GutPillars;

public class GutPillarsViewModel
{
    public GutPillarsViewModel(IList<UserGutPillars>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserGutPillars).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserGutPillars UserMood { get; init; } = null!;
    public UserGutPillars? PreviousMood { get; init; }
