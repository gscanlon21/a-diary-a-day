...
namespace Web.Views.Shared.Components.CbcWAutoDiff;

public class CbcWAutoDiffViewModel
{
    public CbcWAutoDiffViewModel(IList<UserCbcWAutoDiff>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserCbcWAutoDiff).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserCbcWAutoDiff UserMood { get; init; } = null!;
    public UserCbcWAutoDiff? PreviousMood { get; init; }
