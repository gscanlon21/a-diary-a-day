...
namespace Web.Views.Shared.Components.SerumHeavyMetals;

public class SerumHeavyMetalsViewModel
{
    public SerumHeavyMetalsViewModel(IList<UserSerumHeavyMetals>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumHeavyMetals).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumHeavyMetals UserMood { get; init; } = null!;
    public UserSerumHeavyMetals? PreviousMood { get; init; }
