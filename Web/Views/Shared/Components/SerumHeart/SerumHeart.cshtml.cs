...
namespace Web.Views.Shared.Components.SerumHeart;

public class SerumHeartViewModel
{
    public SerumHeartViewModel(IList<UserSerumHeart>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumHeart).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumHeart UserMood { get; init; } = null!;
    public UserSerumHeart? PreviousMood { get; init; }
