...
namespace Web.Views.Shared.Components.GutShortChainFattyAcids;

public class GutShortChainFattyAcidsViewModel
{
    public GutShortChainFattyAcidsViewModel(IList<UserGutShortChainFattyAcids>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserGutShortChainFattyAcids).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserGutShortChainFattyAcids UserMood { get; init; } = null!;
    public UserGutShortChainFattyAcids? PreviousMood { get; init; }
