...
namespace Web.Views.Shared.Components.GutConditionalBacteria;

public class GutConditionalBacteriaViewModel
{
    public GutConditionalBacteriaViewModel(IList<UserGutConditionalBacteria>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserGutConditionalBacteria).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserGutConditionalBacteria UserMood { get; init; } = null!;
    public UserGutConditionalBacteria? PreviousMood { get; init; }
