...
namespace Web.Views.Shared.Components.SerumImmuneRegulation;

public class SerumImmuneRegulationViewModel
{
    public SerumImmuneRegulationViewModel(IList<UserSerumImmuneRegulation>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumImmuneRegulation).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumImmuneRegulation UserMood { get; init; } = null!;
    public UserSerumImmuneRegulation? PreviousMood { get; init; }
