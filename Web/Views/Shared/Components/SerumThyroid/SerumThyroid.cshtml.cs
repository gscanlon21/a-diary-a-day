...
namespace Web.Views.Shared.Components.SerumThyroid;

public class SerumThyroidViewModel
{
    public SerumThyroidViewModel(IList<UserSerumThyroid>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumThyroid).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumThyroid UserMood { get; init; } = null!;
    public UserSerumThyroid? PreviousMood { get; init; }
