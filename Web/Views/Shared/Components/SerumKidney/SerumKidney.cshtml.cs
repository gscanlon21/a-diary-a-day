...
namespace Web.Views.Shared.Components.SerumKidney;

public class SerumKidneyViewModel
{
    public SerumKidneyViewModel(IList<UserSerumKidney>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumKidney).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumKidney UserMood { get; init; } = null!;
    public UserSerumKidney? PreviousMood { get; init; }
