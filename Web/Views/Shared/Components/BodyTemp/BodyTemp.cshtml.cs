...
namespace Web.Views.Shared.Components.BodyTemp;

public class BodyTempViewModel
{
    public BodyTempViewModel(IList<UserBodyTemp>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value,
                    Description = typeof(UserBodyTemp).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
