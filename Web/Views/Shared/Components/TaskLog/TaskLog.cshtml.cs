...
public class TaskLogViewModel
{
    public TaskLogViewModel(Data.Entities.Users.User user, UserTask userTask, IList<UserTaskLog>? userLogs)
    {
        ...
        if (userLogs != null)
        {
            ...
            {
                var totalDays = Enumerable.Range(0, userTask.ChartDays).Select(i => DateHelpers.Today.AddDays(-i)).Distinct();
            ...
            {
                var totalWeeks = Enumerable.Range(0, userTask.ChartDays).Select(i => DateHelpers.Today.AddDays(-i).StartOfWeek()).Distinct();
