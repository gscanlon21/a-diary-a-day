...
namespace Web.Views.Shared.Components.Urine;

public class UrineViewModel
{
    public UrineViewModel(IList<UserUrine>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserUrine).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserUrine UserMood { get; init; } = null!;
    public UserUrine? PreviousMood { get; init; }
