...
namespace Web.Views.Shared.Components.SerumBlood;

public class SerumBloodViewModel
{
    public SerumBloodViewModel(IList<UserSerumBlood>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumBlood).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumBlood UserMood { get; init; } = null!;
    public UserSerumBlood? PreviousMood { get; init; }
