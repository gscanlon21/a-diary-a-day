...
namespace Web.Views.Shared.Components.CompleteMetabolicPanel;
    ...
    public CompleteMetabolicPanelViewModel(IList<UserCompleteMetabolicPanel>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserCompleteMetabolicPanel).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
