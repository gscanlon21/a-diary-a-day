...
namespace Web.Views.Shared.Components.SerumAutoimmunity;

public class SerumAutoimmunityViewModel
{
    public SerumAutoimmunityViewModel(IList<UserSerumAutoimmunity>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumAutoimmunity).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumAutoimmunity UserMood { get; init; } = null!;
    public UserSerumAutoimmunity? PreviousMood { get; init; }
