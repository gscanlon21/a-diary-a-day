...
namespace Web.Views.Shared.Components.SerumMaleHealth;

public class SerumMaleHealthViewModel
{
    public SerumMaleHealthViewModel(IList<UserSerumMaleHealth>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumMaleHealth).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumMaleHealth UserMood { get; init; } = null!;
    public UserSerumMaleHealth? PreviousMood { get; init; }
