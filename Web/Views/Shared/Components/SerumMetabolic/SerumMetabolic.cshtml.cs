...
namespace Web.Views.Shared.Components.SerumMetabolic;

public class SerumMetabolicViewModel
{
    public SerumMetabolicViewModel(IList<UserSerumMetabolic>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumMetabolic).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumMetabolic UserMood { get; init; } = null!;
    public UserSerumMetabolic? PreviousMood { get; init; }
