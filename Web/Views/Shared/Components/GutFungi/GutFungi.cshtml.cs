...
namespace Web.Views.Shared.Components.GutFungi;

public class GutFungiViewModel
{
    public GutFungiViewModel(IList<UserGutFungi>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserGutFungi).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserGutFungi UserMood { get; init; } = null!;
    public UserGutFungi? PreviousMood { get; init; }
