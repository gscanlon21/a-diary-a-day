...
namespace Web.Views.Shared.Components.SerumElectrolytes;

public class SerumElectrolytesViewModel
{
    public SerumElectrolytesViewModel(IList<UserSerumElectrolytes>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumElectrolytes).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumElectrolytes UserMood { get; init; } = null!;
    public UserSerumElectrolytes? PreviousMood { get; init; }
