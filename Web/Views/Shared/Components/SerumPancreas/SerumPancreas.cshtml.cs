...
namespace Web.Views.Shared.Components.SerumPancreas;

public class SerumPancreasViewModel
{
    public SerumPancreasViewModel(IList<UserSerumPancreas>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumPancreas).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumPancreas UserMood { get; init; } = null!;
    public UserSerumPancreas? PreviousMood { get; init; }
