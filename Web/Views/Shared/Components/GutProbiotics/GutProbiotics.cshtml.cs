...
namespace Web.Views.Shared.Components.GutProbiotics;

public class GutProbioticsViewModel
{
    public GutProbioticsViewModel(IList<UserGutProbiotics>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserGutProbiotics).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserGutProbiotics UserMood { get; init; } = null!;
    public UserGutProbiotics? PreviousMood { get; init; }
