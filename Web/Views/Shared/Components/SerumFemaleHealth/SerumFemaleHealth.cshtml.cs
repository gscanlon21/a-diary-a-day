...
namespace Web.Views.Shared.Components.SerumFemaleHealth;

public class SerumFemaleHealthViewModel
{
    public SerumFemaleHealthViewModel(IList<UserSerumFemaleHealth>? userMoods, List<UserCustom>? customs)
    {
        if (userMoods != null && customs != null)
        {
            var flatMap = userMoods.SelectMany(m =>
            {
                ...
                {
                    Value = c.Value ?? 0,
                    Description = typeof(UserSerumFemaleHealth).GetProperty(c.Key)!.GetCustomAttribute<DisplayAttribute>()!.Description
                });
            });
    ...
    public UserSerumFemaleHealth UserMood { get; init; } = null!;
    public UserSerumFemaleHealth? PreviousMood { get; init; }
