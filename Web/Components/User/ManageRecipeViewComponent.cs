...
public class ManageRecipeViewComponent(CoreContext context, IServiceScopeFactory serviceScopeFactory) : ViewComponent
{
    ...
    public async Task<IViewComponentResult> InvokeAsync(Data.Entities.User.User user, UserTask recipe, UserManageRecipeViewModel.Params parameters)
    {
        var userRecipe = await context.UserTasks.AsNoTracking().FirstOrDefaultAsync(r => r.UserId == user.Id && r.Id == parameters.RecipeId);
        if (userRecipe == null)
        {
            return Content("");
        }
        ...
        var recipeDto = (await new QueryBuilder(Section.None)
            ...
            .FirstOrDefault(r => r.Task.Id == recipe.Id);
